...
try:
    ...
    if hasattr(mp, 'solutions'):
        ...
        logger.info("MediaPipe initialized successfully (solutions API)")
    else:
        # Newer MediaPipe versions - try alternative imports
        try:
            ...
            MEDIAPIPE_AVAILABLE = True
            logger.info("MediaPipe initialized successfully (direct imports)")
...
def analyze_movement(frame: np.ndarray, exercise_type: str) -> Dict[str, Any]:
    ...
    if exercise_type in ['body', 'balance', 'coordination', 'general'] and MEDIAPIPE_AVAILABLE and mp_pose:
        try:
            with mp_pose.Pose(
    ...
    if exercise_type in ['hands', 'fingers', 'fine_motor', 'general'] and MEDIAPIPE_AVAILABLE and mp_hands:
        try:
            with mp_hands.Hands(
                static_image_mode=True,
                max_num_hands=2,
    ...
    if not result['pose_detected'] and not result['hands_detected']:
        if not MEDIAPIPE_AVAILABLE:
            result['feedback'] = 'Movement analysis unavailable: MediaPipe library not properly initialized.'
        else:
